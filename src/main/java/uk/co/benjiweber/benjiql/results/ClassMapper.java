...
public class ClassMapper<T> implements Mapper<T> {
    private final T t;
    private final Set<SettableField> properties = new HashSet<>();
    private final Recorder<T> recorder;

    public ClassMapper(Supplier<T> factory) {
        this.t = factory.get();
        recorder = (Recorder<T>)RecordingObject.create(t.getClass());
    }

    public static <T> ClassMapper<T> mapper(Supplier<T> factory) {
        return new ClassMapper<T>(factory);
    }

    public <U extends Serializable> ClassMapper<T> set(BiConsumer<T,U> setter) {
